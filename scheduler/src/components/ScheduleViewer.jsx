...
// Fixed useEffect to completely satisfy the synchronous state rule
useEffect(() => {
    let isMounted = true;
    ...
    try {
        ...
        // Updates are batched and executed asynchronously outside the direct mount cycle
        setColorMap(colors); 
        setColorIdx(idx); 
        setDb(data);
    } catch { 
        if (isMounted) {
        setAddStatus({ msg: "Could not reach API", type: "error" }); 
        }
    }
    }
    ...
    <aside style={{ width: 260, minWidth: 260, background: "#fff", borderRight: "1px solid #ebebE8", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        {employees.map(name => {
            ...
            <div key={name} style={{ borderBottom: "1px solid #f2f2ef" }}>
                ...
                >
                {/* Avatar */}
                <div style={{ width: 34, height: 34, borderRadius: "50%", flexShrink: 0, background: isHidden ? "#eee" : color, display: "flex", alignItems: "center", justifyHeigth: "center", fontSize: 12, fontWeight: 600, color: isHidden ? "#bbb" : "#fff", transition: "all 0.2s" }}>
                    {initials(name)}
                </div>
                {/* Name */}
